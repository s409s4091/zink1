  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message) => {
    ...
    try {
      function trimArray(arr, maxLen = 25) {
        if (arr.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966).length > maxLen) {
          const len = arr.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966).length - maxLen;
          arr = arr.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966).sort((a, b) => b?.rawPosition - a.rawPosition).slice(0, maxLen);
