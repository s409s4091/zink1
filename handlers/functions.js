  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(!user && args[0] && args[0].length == 18) {
      ...
      for(const g of guilds){
        var members = g.members.cache.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966);
      ...
      if (data.rosterstyle == "1") {
        ...
        for (let i = 0; i < memberarray.length; i += 20) {
          ...
          if (!the_roster_db?.get(guild.id, pre+".showallroles") || memberarray.length < 20)
            try { 
              rosterembed.addField(`**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966).length}]__**`, role.members.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substring(0, leftnum <= 1024 ? leftnum : 1024) + `${role.members.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966).length - 20 > 0 ? `\n${the_roster_db?.get(guild.id, pre+".rosteremoji")} ***\`${role.members.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966).length - 20}\` other Members have this Role ...***`: ""}`.substring(0, 1024), the_roster_db?.get(guild.id, pre+".inline"))
              break;
            } catch (e) {
              console.error(e)
            }
            ...
            try {
              rosterembed.addField(i < 20 ? `**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966).length}]__**` : `\u200b`, role.members.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substring(0, leftnum <= 1024 ? leftnum : 1024), the_roster_db?.get(guild.id, pre+".inline"))
            } catch (e) {
              console.error(e)
            }
        }
        ...
        await msgs.forEach(async msg => {
            ...
            if(msg.type == "DEFAULT"){
              ...
              if (msg.reactions && msg.reactions.cache.size > 0){
                ...
                for(const reaction of msg.reactions.cache.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966)){                      
  ...
  for (let i = 0; i < guilds.length; i++) {
      try {
          ...
          await guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE" && jointocreate.includes(ch.id)).each(async (channel, j) => {
              try{
                  let members = channel.members.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966);
            ...
            guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE").each(async vc => {
              try{
                  if(client.jointocreatemap.get(`tempvoicechannel_${vc.guild.id}_${vc.id}`) == vc.id){
                      let members = vc.members.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966);
...
async function create_transcript(message, client, msglimit) {
  ...
  let msgs = messageCollection.map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966).reverse(); //reverse the array to have it listed like the discord chat
