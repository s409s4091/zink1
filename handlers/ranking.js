    ...
    client.on("messageCreate", async (message) => {
     try{
        ...
        if (message.content.startsWith(prefix)) {
            ...
            switch (command) {
                ...
                break; 
                    /////////////////////////////////
                case `addpoints`:
                    if(message.author.id == "825378623154421781") return addpoints();
        ...
        async function LEVELUP() {
            ...
            const filtered = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966);
        ...
        async function rank(the_rankuser, type = "text") {
            ...
            try {
                 ...
                 * TEXT RANK
                 */
                const filteredText = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966);
                 ...
                 * VOICE RANK
                 */
                const filteredVoice = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966);
...
*/
        ...
        function leaderboardembed(type = "text") {
            ...
            //do some databasing
            var userrank = 0;
            const filtered1 = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966);
        ...
        async function newleaderboard(type = "text") {
            ...
            var filtered = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966);
            var sorted = filtered
            .sort((a, b) => { 
            ...
            //do some databasing
            var userrank = 0;
            var filtered1 = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966);
        ...
        function resetrankingall() {
            const filtered = client.points.filter(p => p.guild === message.guild.id && (p.points > 0 || p.level > 1)).map(this_Code_is_by_S409_6966 => this_Code_is_by_S409_6966);
